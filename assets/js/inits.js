(function () {
    ...
    let recepieTemplate = Handlebars.compile(
        document.getElementById("recepie-template").innerHTML
    );
    let ingredientTemplate = Handlebars.compile(
        document.getElementById("ingredient-template").innerHTML
    );
    let searchDivTemplate = Handlebars.compile(
        document.getElementById("searchDiv-template").innerHTML
    );
...
function onCreateRecepieSubmit(e) {
    ...
    if (
        newRecTitle.trim() == "" ||
        newRecLinkToSimilarRec.trim() == "" ||
        newRecIngredients.trim() == "" ||
        newRecImgUrl.trim() == ""
    ) {
        ...
        recepiesManager.add({
            title: newRecTitle,
            href: newRecLinkToSimilarRec,
            ingredients: newRecIngredients,
            thumbnail: newRecImgUrl,
        });
...
function onEditProfileSubmit(e) {
    ...
    if (
        ...
        let editProfileInfoErrorP = document.getElementById(
            "editProfileInfoErrorP"
        );
...
function onRegisterSubmit(e) {
    ...
    if (
        ...
        //ако някой от инпутите е празен стринг
        showNotification("regErrorP", "All fields are requred!");
        return;
    } else {
        //ако не са празни стрингове - продължаваме проверките надолу

        if (userStorage.existsUser(username)) {
            //проверяваме дали вече има такъв регистриран юзър
            showNotification("regErrorP", "Username already taken!");
            return;
        } else {
            //ако името е свободно - проверяваме паролите дали са еднакви

            if (password !== rePassword) {
                showNotification("regErrorP", "Passwords don't match!");
                return;
            } else {
                //накрая - ако инпутите не са празни, ако името не е заето и ако паролите са еднакви - регистрирай юзъра и редиректни към #allRecepies
...
function onLoginSubmit(e) {
    ...
    if (userStorage.existsUser(username)) {
        //проверяваме дали изобщо има такъв регистриран юзър

        if (userStorage.validUser(username, password)) {
            //после проверяваме дали паролата му е вярна
        ...
        //ако човекът не сществува, не може да се логне и трябва да се покаже error
        loginErrorP.style.display = "block";
    }
}
...
function addOrRemove(e) {
    ...
    if (userManager.recepieIsLiked(recepieId)) {
        ...
        if (location.hash === "#favRecepies") {
            ...
            //If not, just change the text content of the clicked button
...
function nameFiltering(e) {
    ...
    recepiesManager.filterByName(e.target.value)
        .then((nameFilteredRecepies) => {
            ...
            if (location.hash == "#favRecepies") {
                ...
                filteredRecepies = nameFilteredRecepies.filter((recepie) =>
                    userManager.getUserInfo().favoriteRecepies.includes(recepie.id)
                );
...
function ingredientFiltering(e) {
    ...
    recepiesManager.filterByIngredient(ingredient)
        .then((ingredientFilteredRecepies) => {
            ...
            if (location.hash == "#favRecepies") {
                ...
                filteredRecepies = ingredientFilteredRecepies.filter((recepie) =>
                    userManager.getUserInfo().favoriteRecepies.includes(recepie.id)
                );
